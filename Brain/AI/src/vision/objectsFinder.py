...
class ObjectsFinder:
    ...
    def __init__(self,channel, debug=False, testScreenshot=None):
        #
        ...
        self.__img_matrix = getImg(os.path.join(SCREENSHOT_PATH, screenshot),channel) 
    ...
    def find_all_gray_scale(self, elements_to_find:{}, request_regmax:True)->dict:
        image_gray = cv2.cvtColor(self.__img_matrix, cv2.COLOR_BGR2GRAY)
        objects_found = self.find_all_among(image_gray,elements_to_find,request_regmax)
    ...
    def find_circles(self, balls_min_distance, balls_min_radius, balls_max_radius) -> list:      
        output = self.__img_matrix.copy()  
        blurred = cv2.GaussianBlur(self.__img_matrix, (65, 65), 0)  # Used to find the color of the balls
        ...
        if circles is not None:
            ...
            for (x, y, r) in circles:
                ...
                cv2.circle(blurred, (x, y), 6, (0, 0, 0), 1)
                cv2.putText(output, f"({x}, {y})", (x + 10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
