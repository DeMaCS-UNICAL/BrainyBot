...
from AI.src.gfd_code_updated.EdgeDetectionTechniques2 import retain_candy_region
from AI.src.gfd_code_updated.extrusion_remover import process_image
from AI.src.gfd_code_updated.autotuning import  genetic_algorithm_tune_parameters
from AI.src.gfd_code_updated.HSV_Horizontal_Vertical_merged_ImageMatching2 import process_image_with_parameters,get_grid
import time
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--path", type=str, required=True, help="Path to the image images.")
    parser.add_argument("--population", type=int, required=True, help="Size of the population for GA.")
    parser.add_argument("--generations", type=int, required=True, help="Number of generations without improvements for GA.")
    parser.add_argument("--tolerance", type=float, required=True, help="Tolerance for the fitness evaluation for GA.")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("-g", "--gamename", type=str, required=False, help="Name of the game being processed.")
    parser.add_argument("-s", "--save_ids", action="store_true", help="Add this flag to save txt IDs (default: False).")
    group.add_argument("-t", "--tune", type=str, required=False, help="Name of the game to be tuned.")
    group.add_argument("-d", "--debug", action="store_true", help="Only show the retain candy region, no grid detection.")
