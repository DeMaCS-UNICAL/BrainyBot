...
from AI.src.constants import SCREENSHOT_PATH, USE_ADB, CLIENT_PATH, TAPPY_ORIGINAL_SERVER_IP, logger
...
def get_screenshot(url = None, port = None, to_memory: bool = False, save_path: str = None, filename: str = "screenshot.png") -> bool | np.ndarray:
    """
    ...
    try:
        match (USE_ADB, to_memory):
            case (True, True): return run_adb_screencap_to_memory()
            case (True, False): return run_adb_screencap_to_memory(
                file_path=os.path.join(SCREENSHOT_PATH, filename) if save_path is None else save_path,
                filename=filename
            )
            ...
            case (False, False): return require_image_from_url(
                url=url,
                port=port,
                save_path=os.path.join(SCREENSHOT_PATH, filename),
                filename=filename
            )
...
def require_image_from_url(url, port, save_path: str = None, filename: str = "screenshot.png") -> bool:
...
def require_image_from_adb(save_path: str = None, filename: str = "screenshot.png") -> bool:
    if save_path is None:
        save_path = SCREENSHOT_PATH
    if not os.path.exists(save_path):
        raise Exception(f"The directory {save_path} does not exist")
...
def run_adb_screencap_to_memory(slow_usb: bool = True, file_path: str = None, filename: str = "screenshot.png") -> np.ndarray:
    """
