...
def np_absolute_distance_image_comparison(img0: Image.Image | np.ndarray, img1: Image.Image | np.ndarray) -> tuple[
    float, int]:
    """
...
def cv2_structural_similarity(img0: np.ndarray, img1: np.ndarray) -> float:
    ...
    """
    first_gray = cv2.cvtColor(to_uint8(img0), cv2.COLOR_RGBA2GRAY)
    second_gray = cv2.cvtColor(to_uint8(img1), cv2.COLOR_RGBA2GRAY)
...
def check_benchmarked_algorithms(threads: list[SimilarityThread]) -> None:
    def find_min(scores):
        min_score = float("inf")
        for s in scores:
            if s < min_score:
    ...
    t4 = sum((t.times[2] for t in threads), datetime.timedelta()) / len(threads)
    ...
    score4 = find_min([t.scores[2] for t in threads])
    ...
    shift4 = [t.shift for t in threads if t.scores[2] == score3][0]
...
if __name__ == "__main__":
    ...
    os.chdir("../resources")
    mask = np.array(Image.open("islandempire_mask_alpha.png"))

    a = np.array(Image.open("screenshot_0.png"))
    b = np.array(Image.open("screenshot_0.png"))
    print(np_absolute_distance_image_comparison(a, b))
