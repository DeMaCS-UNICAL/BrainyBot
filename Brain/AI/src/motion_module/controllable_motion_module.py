...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.desk_title_label = customtkinter.CTkLabel(self.desk_frame, text="Desk (Map)")
        self.desk_title_label.pack(padx=10, pady=2)
        
        self.desk_canvas = tk.Canvas(self.desk_frame, bg="#212121", highlightthickness=0)
        ...
        self.control_label = customtkinter.CTkLabel(self.right_frame, text="Controls",
        ...
        self.position_label = customtkinter.CTkLabel(self.right_frame, text="Position: (0, 0)",
                                                     font=customtkinter.CTkFont(size=16, weight="bold"))
        self.position_label.pack(side="bottom", padx=10, pady=10)
        
        self.status_label = customtkinter.CTkLabel(self.right_frame, text="Status: Idle", wraplength=200)
    ...
    def __movement_group(self):
        self.group1_label = customtkinter.CTkLabel(self.right_frame, text="Movement",
                                                   font=customtkinter.CTkFont(size=14, weight="bold"))
        self.group1_label.pack(padx=10, pady=(15, 5))
        
        self.btn_goto = customtkinter.CTkButton(self.right_frame, text="Goto(x, y)", command=self.cmd_goto)
        self.btn_goto.pack(padx=20, pady=5, fill="x")
        
        self.btn_goto_action = customtkinter.CTkButton(self.right_frame, text="Goto For Action(x, y)",
                                                       command=self.cmd_goto_action)
        self.btn_goto_action.pack(padx=20, pady=5, fill="x")
        
        self.btn_move = customtkinter.CTkButton(self.right_frame, text="Move(dx, dy)", command=self.cmd_move)
        self.btn_move.pack(padx=20, pady=5, fill="x")
    
    def __position_and_desk_group(self):
        self.group2_label = customtkinter.CTkLabel(self.right_frame, text="Position & Desk",
                                                   font=customtkinter.CTkFont(size=14, weight="bold"))
        self.group2_label.pack(padx=10, pady=(15, 5))
        
        self.btn_check_pos = customtkinter.CTkButton(self.right_frame, text="Check Position Changed",
                                                     command=self.cmd_check_position)
        self.btn_check_pos.pack(padx=20, pady=5, fill="x")
        
        self.btn_set_pos = customtkinter.CTkButton(self.right_frame, text="Set Position(x, y)",
                                                   command=self.cmd_set_position)
        self.btn_set_pos.pack(padx=20, pady=5, fill="x")
        
        self.btn_collapse = customtkinter.CTkButton(self.right_frame, text="Collapse Desk",
                                                    command=self.cmd_collapse_desk)
        self.btn_collapse.pack(padx=20, pady=5, fill="x")
        
        self.btn_reset_desk = customtkinter.CTkButton(self.right_frame, text="Reset Desk", command=self.cmd_reset_desk)
        self.btn_reset_desk.pack(padx=20, pady=5, fill="x")
    
    def __configuration_group(self):
        self.group3_label = customtkinter.CTkLabel(self.right_frame, text="Configuration",
                                                   font=customtkinter.CTkFont(size=14, weight="bold"))
        self.group3_label.pack(padx=10, pady=(15, 5))
        
        self.btn_select_mask = customtkinter.CTkButton(self.right_frame, text="Select Mask",
                                                       command=self.cmd_select_mask)
        self.btn_select_mask.pack(padx=20, pady=5, fill="x")
        
        self.btn_select_calib = customtkinter.CTkButton(self.right_frame, text="Select Calibration",
        ...
        self.calib_dropdown = customtkinter.CTkOptionMenu(self.right_frame,
                                                          values=["linear_regression", "linear_interpolation", "ransac_regression", "huber_regression"],
                                                          variable=self.calib_method_var)
        self.calib_dropdown.pack(padx=20, pady=5, fill="x")
        
    def __extra_group(self):
        self.group4_label = customtkinter.CTkLabel(self.right_frame, text="Extra",
                                                   font=customtkinter.CTkFont(size=14, weight="bold"))
        self.group4_label.pack(padx=10, pady=(15, 5))
        
        self.btn_save_desk = customtkinter.CTkButton(self.right_frame, text="Save Desk",
                                                     command=self.cmd_save_desk)
        ...
        self.entry_screen_interval = customtkinter.CTkEntry(self.screen_interval_frame, width=60)
        self.entry_screen_interval.pack(side="left", padx=5)
        self.entry_screen_interval.insert(0, str(self.screen_update_interval))
        
        self.btn_set_screen_interval = customtkinter.CTkButton(self.screen_interval_frame, text="Set Screen Interval",
        ...
        self.entry_desk_interval = customtkinter.CTkEntry(self.desk_interval_frame, width=60)
        self.entry_desk_interval.pack(side="left", padx=5)
        self.entry_desk_interval.insert(0, str(self.desk_update_interval))
        
        self.btn_set_desk_interval = customtkinter.CTkButton(self.desk_interval_frame, text="Set Desk Interval",
        ...
        if desk is not None:
            try:
                ...
                if filepath:
                    ...
                    self.status_label.configure(text=f"Status: Desk saved to {os.path.basename(filepath)}")
            except Exception as e:
    ...
    def cmd_set_screen_interval(self):
        try:
            ...
            if val > 0:
                ...
                self.status_label.configure(text=f"Status: Screen interval set to {val}s")
            else:
    ...
    def cmd_goto(self):
        ...
        if x is not None:
            ...
            threading.Thread(target=lambda: self.motion_module.goto((x, y)), daemon=True).start()
    
    def cmd_goto_action(self):
        x, y = self.get_inputs()
        if x is not None:
            self.status_label.configure(text=f"Status: Executing GotoAction({x}, {y})")
            threading.Thread(target=lambda: self.motion_module.goto_for_action((x, y)), daemon=True).start()
    
    def cmd_move(self):
        dx, dy = self.get_inputs()
        if dx is not None:
            self.status_label.configure(text=f"Status: Executing Move({dx}, {dy})")
            threading.Thread(target=lambda: self.motion_module.move_with_offset((dx, dy)), daemon=True).start()
    ...
    def cmd_check_position(self):
        ...
        def run():
            try:
                ...
                self.after(0, lambda: self.status_label.configure(text=f"Status: Position Changed? {changed}"))
            except Exception as e:
                self.after(0, lambda: self.status_label.configure(text=f"Status: Error {e}"))
    ...
    def process_desk_click(self, event):
        ...
        self.status_label.configure(text=f"Desk: ({desk_x}, {desk_y}) | Pos: ({pos_x}, {pos_y})")
    ...
    def on_screen_click(self, event):
        if not hasattr(self.image_label, '_original_size') or not hasattr(self.image_label, '_display_size'):
            return
        ...
        print(f"Debug Screen Click: widget={widget_w}x{widget_h} img={img_w}x{img_h} event={event.x},{event.y} calc={x_on_img},{y_on_img}")
        self.status_label.configure(text=f"Desk: {desk_coord} | Screen: ({screen_x}, {screen_y}) | Pos: ({pos_x}, {pos_y})")
    ...
    def _display_screenshot(self, img):
        try:
            ...
            ctk_image = customtkinter.CTkImage(light_image=img, dark_image=img, size=(target_w, target_h))
    ...
    def _display_desk(self, img):
        try:
            ...
            if self.desk_image_item is None:
                ...
                self.desk_image_item = self.desk_canvas.create_image(x, y, image=self.desk_tk_image, anchor="nw")
            else:
                self.desk_canvas.itemconfig(self.desk_image_item, image=self.desk_tk_image)
        except Exception as e:
