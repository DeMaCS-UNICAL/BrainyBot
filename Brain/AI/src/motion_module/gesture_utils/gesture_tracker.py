...
class Gesture:
    ...
    def __str__(self):
        ...
        if len(self.points) > 10:
            points_str = (
                f"[{self.points[0]}, ..., {self.points[-1]}] ({len(self.points)} items)"
            )

        return (
            f"[⌚] {time.ctime(self.start_time)}\n"
...
class GestureTracker(threading.Thread, AbstractContextManager):
    ...
    def __init__(
        self, output_queue: queue.Queue = queue.Queue(), live_feed: bool = False
    ):
        ...
        self.__data_x, self.__data_y = extract_phone_screen_info()
        self.__scale_x: float = self.__data_x[2]
        self.__scale_y: float = self.__data_y[2]
    ...
    def run(self):
        ...
        try:
            ...
            for raw_line in iter(self.__process.stdout.readline, b""):
                ...
                if match_x:
                    curr_x = int(match_x.group(1), 16) // self.__scale_x
                    last_valid_x = curr_x
                    update_coords = True

                match_y = self.__re_y.search(line)
                if match_y:
                    curr_y = int(match_y.group(1), 16) // self.__scale_y
                    last_valid_y = curr_y
                    update_coords = True

                if active_touch and update_coords:
                    if not gesture_points or gesture_points[-1] != (
                        last_valid_x,
                        last_valid_y,
                    ):
